			...
			<div class="col-6 offset-3">
				<form action="<?=base_url().'premio/updatePost';?>" method="post" >
					...
					<div class="form-group">
						...
						<input type="number" min="1" class="form-control" id="numero" name="numero" value="<?= $premio->numero; ?>" placeholder="Introduce número"/>
					</div>
					<div class="form-group">
						<label for="idTipo" class="font-weight-bold">Selecciona Tipo de Premio</label><br />
						<select name="idTipo" class="form-control">
							<?php foreach($tipos as $tipo): ?>
								<option value="<?=$tipo->id?>" <?= ($premio->tipo_id == $tipo->id) ? 'selected':'';?>><?=$tipo->nombre?></option>
							<?php endforeach; ?>
						</select>
					</div>
					<div class="form-row">
